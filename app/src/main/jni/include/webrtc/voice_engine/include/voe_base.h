...
{
public:
	int mAyVcID;			//调用AiyouVoiceRpm::StartCapture的时候会把ID填入这个字段,为了使在结束采集的时候从回调列表中删除
	bool mAyVcIsUseAec;	//是否支持回声消除，支持的话，使用webrtc的回声消除,1表示使用，其他值表示不使用
	bool mIscbed;	 	//是否已经回调数据了
	/*采样率8000, 16000, 24000, 44100, 48000, 
	采集出来的是48000，回调回来的数据不一定是你设定的值，
	但是会在CapDataCallback的sampleRate中回传给你正确的采样率
	*/
	int mAyVcSampleRate; 
	virtual int32_t CapDataCallback(char *pData, int len, int sampleRate, int bits, int channelNumb)=0;
	CAyVoiceCapCb()
	:mAyVcID(-1),
	mAyVcIsUseAec(false),
	mAyVcSampleRate(8000)
	{
	}
};
...
public:
	...
	virtual int32_t StartPlayoutPcm(int sampleRate)=0;
    virtual int32_t StopPlayoutPcm(int chanid)=0;
    virtual int32_t StartCapturePcm(CAyVoiceCapCb *pCapDataCb)=0;
    virtual int32_t StopCapturePcm(int id)=0;

	virtual int32_t OnPcmDataIn(void *pData, int len, int chanid, long timestamp)=0;
