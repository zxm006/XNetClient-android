 ...
 *
 */
public class TestConfJoinActivity extends Activity implements OnClickListener {
	private final String TAG = TestConfJoinActivity.class.getSimpleName();

	private ConfManageUtil mConfMgr;
	private Button mBtnLogin;

	private Button mBtnConfCreate;
	private EditText mTvConfID;
...
//		requestWindowFeature(Window.FEATURE_NO_TITLE);
		...
		setContentView(R.layout.activity_conf_join);
		...
		mTvConfID = (EditText) findViewById(R.id.tv_confid);
		...
		public boolean onLongClick(View v) {
			for (int i = 0; i < mArrDisView.length; i++) {
				if (mArrDisView[i].mSurface.getId() == v.getId()) {
					...
					final TextView tv = new TextView(TestConfJoinActivity.this);
					String tmp = "确定要踢出" + peerNumb + "吗?";
					tv.setText(tmp);

					new AlertDialog.Builder(TestConfJoinActivity.this).setTitle("踢出成员")
	...
	ConfEventCallback mCallback = new ConfEventCallback() {
		...
		public int onConfInviteIn(String userid, String confid) {
			...
			Handler hand = new Handler(getMainLooper()) {
				...
				public void handleMessage(Message msg) {
					...
					new AlertDialog.Builder(TestConfJoinActivity.this).setTitle("会议邀请")
							.setIcon(android.R.drawable.ic_dialog_info).setView(tv)
							.setPositiveButton("确定", new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog, int which) {
									mStrConfId = fConfID;
		...
		public int onConfStatusChanged(int status, String peerName, String head, String nick, String jingdu, String weidu, String tm) {
			Message msg = new Message();
			Log.i(TAG, "conf statu cb:" + status + "," + peerName+"," + nick + "," +head+","+jingdu+","+weidu+","+tm);
	...
	public void onClick(View view) {
		if (view.getId() == mBtnLogin.getId()) {
			if (!mbIsLogined) {
				...
				mMyNumber = user.getText().toString();
			...
			if (TextUtils.isEmpty(mStrConfId)) {
				mStrConfId = mTvConfID.getText().toString();
				if(TextUtils.isEmpty(mStrConfId))
				{
					Toast.makeText(this, "请输入会议号码", 1).show();
					return;
				}
				mConfMgr.confJoin(mStrConfId, true);
				mBtnConfCreate.setText("退出会议");
			} else {
				if (mIsMaster) {
